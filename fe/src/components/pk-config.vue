<template>
    <div class="pk-config"
         :id="`pk-config-${_uid}`">
        <div class=" jumbotron">
            <h1 v-html="$t('config_greeting')"></h1>
            <p>{{ $t('config_description') }} </p>
        </div>
        <div class="panel panel-primary">
            <div class="panel-heading">
                <h3 class="panel-title">{{ $t('config_panel_title') }}</h3>
            </div>
            <div class="panel-body">
                <div class="form-horizontal">
                    <div class="form-group">
                        <label :for="`pk-config-locale-${_uid}`"
                               class="col-sm-2 control-label">{{ $t('config_label_language') }}</label>
                        <div class="col-sm-1">
